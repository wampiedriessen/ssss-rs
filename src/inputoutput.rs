...
impl InputOutput {
    ...
    pub(crate) fn get_output(&self) -> Result<Box<dyn Write>, String> {
        if let Some(file) = &self.output {
            Ok(Box::new(std::fs::File::create(file).map_err::<String, _>(|_| "Could not open output file!".into())?))
        }
    ...
    pub(crate) fn get_input(&self) -> Result<Box<dyn Read>, String> {
        if let Some(file) = &self.input {
            Ok(Box::new(std::fs::File::open(file).map_err::<String, _>(|_| "Could not open input file!".into())?))
        }
